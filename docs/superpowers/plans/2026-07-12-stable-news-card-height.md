...
- **One deliberate deviation from the spec's exact wording:** the spec's section 2 describes `.panel` getting `overflow: hidden` as a safety clamp. This plan does **not** add `overflow: hidden` to `.panel` at all. Reason: Task 2's popover needs to render outside `.title`'s own `overflow: hidden` (required for its line-clamp) — if `.panel` also clipped overflow, a long popover could get cut off. The fixed height itself (with the buffer described below) is the safety net instead. Task 0's review explicitly re-checks this reasoning. Accepted residual risk (confirmed by that review): this safety net does not cover user/OS-level font-scaling overrides (e.g. Chrome's minimum-font-size setting) that enlarge `.meta`/`.actions`/`.status`/popover text beyond the measured buffer — a known limitation of the fixed-height approach itself (the spec's original `overflow: hidden` clamp would only hide the overflow, not prevent the enlarged text from being visually truncated either), accepted as out of scope for this fix.
...
- Confirm the initial loading screen (before the first card renders) is the exact same height as an active card, not shorter.
