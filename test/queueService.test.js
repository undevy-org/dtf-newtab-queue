...
describe("queueService", () => {
  ...
  it("markViewed fetches the next page only when backlog is empty", async () => {
    const { api, service, store } = await createHarness({
      initialState: stateWith({
        current: item(1),
        lastId: 100
  ...
  it("markViewed exhausts without fetching when no pagination cursor remains", async () => {
    const { api, service, store } = await createHarness({
      initialState: stateWith({
        current: item(1),
        lastId: null
    ...
    assert.deepEqual(stored.seenIds, [1]);
    assert.equal(stored.lastId, null);
  ...
  it("continues bounded pagination when a fetched page contains only duplicates", async () => {
    const { api, service, store } = await createHarness({
      initialState: stateWith({
        current: item(1),
        seenIds: [2, 3],
  ...
  it("stops pagination after one empty API page even when it has a cursor", async () => {
    const { api, service, store } = await createHarness({
      initialState: stateWith({
        current: item(1),
        lastId: 100
  ...
  it("exhausts after the bounded pagination limit returns only duplicates", async () => {
    const { api, service, store } = await createHarness({
      initialState: stateWith({
        current: item(1),
        seenIds: [2, 3, 4, 5, 6],
    ...
    assert.deepEqual(stored.seenIds, [2, 3, 4, 5, 6, 1]);
    assert.equal(stored.lastId, 400);
    assert.equal(stored.exhausted, true);
    assert.equal(stored.events.at(-1).type, "empty");
  });

  it("deduplicates already seen, current, and repeated fetched IDs", async () => {
    const { service, store } = await createHarness({
      initialState: stateWith({
        current: item(1),
        seenIds: [7],
    ...
    assert.equal(stored.lastId, 900);
  });
  ...
  it("sets exhausted when the API returns no usable new items and no backlog remains", async () => {
    const { service, store } = await createHarness({
      initialState: stateWith({
        current: item(1),
        seenIds: [2, 3],
      ...
      batches: [{ items: [item(1), item(2), item(3)], lastId: null }]
    });
